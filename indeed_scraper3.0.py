...
class Job(object):
	def __init__(self, title, company, location, hire_date, end_date, description):
		self.title = title
...
def gen_resume(idd, driver):
	...
	try:
        ...
        # Get everything else about the job
		job_titles = work_experience.find_all(class_="rezemp-u-h4")
		content = work_experience.find_all(class_="rezemp-u-h5")
		...
		for i in range(len(job_titles)):
			...
			desc = [p.get_text() for p in content[i].find_all("span")][1:]
			...
			jobs.append(Job(title, company, location, start_date, end_date, description))
	except:
